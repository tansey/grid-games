...
namespace CondorEvolution
{
    public class CondorEvaluator : IGenomeListEvaluator<NeatGenome>
    {
        ...
        const string EXECUTABLE_PATH = "/u/tansey/grid-games/bin/ModelEvaluator.exe";
        ...
        /// </summary>
        public CondorEvaluator(GridGameParameters ggParams,
                                CondorGroup userGroup)
        {
            _params = ggParams;

            // convert to unix pathnames
            string experimentDir = _params.ExperimentPath.Replace('\\', '/');
        ...
        private void createCondorJobs(IList<NeatGenome> genomeList)
        {
            using (TextWriter writer = new StreamWriter(CONDOR_FILE))
            {
                ...
                writer.WriteLine("+ProjectDescription = \"Grid games experiments\"");
